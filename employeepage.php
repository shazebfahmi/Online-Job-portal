...
<a href="employeeloginpage.php">LOG OUT <i class="fa fa-user"></i></a>
...
<div class="row">
	...
	<div class="main" >
		<h4 "color:#0099ff">Welcome <?php echo $_SESSION['name']; ?> you are logged in successfully</h4>

		<form method="post" class="myform" action="employeepage.php" style="width:800px;"  >

		<center>
					 <h2 "color:#99ff66">FIND YOUR NEXT JOB HERE</h2><br/>
					 <p> I  Am Looking for  <select name="work" id="signupbtn2">
							 ...
							 roles <br><br> In <select name="place" id="signupbtn2"  >
										 ...
										 <h4><span  style="font-family:Harlow Solid Italic;	"> Note: Kindly select atleast one, from the above fields. </span>     </h4>
						<br/>
					<input name="searchjob" id="signupbtn"  type="submit" value="SEARCH" style="width:40%;" /><br/><br/>
	...
	if(isset($_POST['searchjob']))
		{
			// echo '<script type="text/javascript"> alert("search button clicked")</script>';
			$work = $_POST['work'];
			$place = $_POST['place'];
			...
			{
				echo '<table id="table1" width="80%" border="1px" cellpadding="5px" cellspacing="5px" bgcolor="pink" style="border-radius:10px; margin:0 auto;" >  ';
				echo "<tr>
						<th>Job ID</th>
						<th>Company Name</th>
						<th>Job Type</th>
	...
	<br>
	<form method="post" class="myform" action="employeepage.php"><center>
			<label style="font-weight:bold;"><b><br><br>Enter the Job ID for which you wish to apply: <br><br></b></label><br>
			<input name="jobid" type="text" class="inputvalues" placeholder="Type Job Id (refer above)" style="align:center;" required/><br><br>
			<input name="sub_btn" type="submit" id ="signupbtn" value="Apply" style="width:35%;"/><br><br>
	</center>
	</form>
		...
		<?php
			if(isset($_POST['sub_btn']))
			{
				...
				$tempq = "select eid from employee where euname = '$empid'";
				$res11 = mysqli_query($con,$tempq);
				if($res11)
				{
					$row = mysqli_fetch_array($res11);
					$t1 = $row['eid'];
					$quer1 = "insert into applied values('$jobid','$t1')";

					$res2 = mysqli_query($con,$quer1);

					if($res2)
					{
						echo '<script type="text/javascript"> alert("You have successfully applied for the job.\n Kindly wait for the respective company to contact you.")</script>';

					}
					else
					{
						echo '<script type="text/javascript"> alert("Cannot apply for the same job more than once.")</script>';
					}
				}
